...
async function listenBlock(blockNumber) {
  ...
  let n = await web3.eth.getBlockUncleCount(result.number).catch(e => {
    console.log('getBlockUncleCount error:', e)
  })
  ...
  if (n > 0) {
    for (let i = 0; i < n; i++) {
      let b = await web3.eth.getUncle(result.number, i).catch((e) => {
        console.log("getUncle", e)
      })
      b.blockNumber = result.number;
      b.position = i;
      b.reward = blockReward.getUncleReward(b.number, b.blockNumber, reward)
